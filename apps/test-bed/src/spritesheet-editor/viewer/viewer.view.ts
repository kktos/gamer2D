...
export class SpritesheetViewerView extends View {
	...
	static EVENT_ANIM_SELECTED = Symbol.for("ANIM_SELECTED");

	private _spritesheet: Signal<SpriteSheet | undefined> = new Signal<SpriteSheet | undefined>(undefined);
	private selectedSprites: Signal<Set<string> | undefined> = new Signal<Set<string> | undefined>(undefined);
	private font: Font;
	private imagePath = "";
	...
	constructor(ctx: ViewContext) {
		...
		const selectedSprite = new Signal<string | undefined>(undefined);
		...
		// const selectedAnim = new Signal<{ name: string; anim: Anim } | undefined>(undefined);
		// selectedAnim.subscribe((animDef) => {
		// 	this.layer.scene.emit(SpritesheetViewerView.EVENT_ANIM_SELECTED, animDef);
		// });
		const selectedAnim = new Signal<string | undefined>(undefined);
	...
	public createAnim(options) {
		...
		const keys = Array.from(this._spritesheet.value.sprites.keys());

		const animSheet: TAnimation = {
	...
	public generateSpritesheet() {
		if (!this._spritesheet.value) return;
		let script = `spritesheet "${this._spritesheet.value.name}" {\n`;
		script += `\timage "${this.imagePath}"\n`;

		script += "\n";

		script += "\tsprites {\n";
		let currentSprite: string | undefined = undefined;
		for (const [name, canvas] of this._spritesheet.value.sprites) {
		...
		script += "\tanimations {\n";
		for (const [name, anim] of this._spritesheet.value.animations) {
		...
		a.href = url;
		a.download = `${this._spritesheet.value.name}.txt`;
		a.click();
